...
---
= Visualizar status do sistema e de cluster do AI Data Engine
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Como administrador de storage, você pode usar ONTAP System Manager para acessar o dashboard e exibir o status de cluster. Este é um bom primeiro passo antes de iniciar suas tarefas administrativas no AI Data Engine (AIDE) ou se suspeitar de algum problema operacional.
...
+
*** Problemas no nó de computação de dados ou de conectividade
...
== Visualizar integridade e utilização do nó de computação de dados

. No painel de navegação, selecione *Cluster* e depois *Overview*.
. Selecione a guia *Data compute*.
+
Esta aba mostra todos os nós de computação de dados no cluster com:
...
. Expanda um nó de computação de dados para abrir a visualização detalhada e verifique:
+
...
. Selecione *Cabeamento* na página *Cluster > Visão geral* para verificar se os nós de computação de dados estão corretamente conectados aos switches do cluster e para identificar quaisquer problemas de porta ou link.
...
** Na aba *Usuários*, verifique quais usuários do AIDE Console têm acesso.
...
+
*** Volumes
*** LIFs com tipo  `Data compute network` (usado para comunicação entre nós de computação de dados e ONTAP)
...
+
** Integridade ou conectividade do nó de computação de dados
** Problemas de rede do mecanismo de dados
** Erros no espaço de trabalho ou na coleta de dados
** Incompatibilidade de versões de software entre ONTAP e o cluster de nós de computação de dados
